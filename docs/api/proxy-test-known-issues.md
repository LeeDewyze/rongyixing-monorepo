...
> **环境**：`pnpm dev:h5:test` · `VITE_API_MODE=proxy` · `VITE_API_BASE_URL=http://app.rtesp.com` · `VITE_API_DOMAIN=rtesp.com` > **记录日期**：2026-06-24  
...
## 4. 因私出行首页入口为空（P1 · 测试环境 MMS）

| 项           | 说明                                                                                                                                                        |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **页面**     | 首页 · 因私出行 Tab                                                                                                                                         |
| **接口**     | `MmsApiHomeUrl-Sitemap-List`（`mms.getSitemaps`）                                                                                                           |
| **Dev 请求** | `POST /__ryx/MmsApiHomeUrl/Sitemap/List`（Vite 转发至 `http://api-mms.<VITE_API_DOMAIN>`）                                                                  |
| **现象**     | 浏览器直接打开上述 URL 为 GET，会失败；应用内 POST 在 `rtesp.com` 上常返回 **HTTP 502**（`api-mms.rtesp.com` 不可达），`visibleProducts` 为空，产品球不展示 |
| **H5 侧**    | 已登录时若 MMS 失败，会回退读取工作台 **因私出行** 分组（与 Legacy 首页同组数据）                                                                           |
| **归属**     | 测试环境 MMS 网关/服务可用性；生产 `api-mms.rongtrip.cn` 可正常 200                                                                                         |

**后续动作（后端）**：修复 `api-mms.rtesp.com` 502；修好后回归因私 Tab 与 `Sitemap-List` 响应。
