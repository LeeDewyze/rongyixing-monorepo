...
- 账号安全页展示已绑定的钉钉账号。
- 绑定动作通过钉钉容器获取 code，再回到融易行页面完成绑定。
...
- 接口说明：根据当前身份和 `SdkType=DingTalk` 判断是否需要引导绑定。
- 请求参数：
...
| 字段 | 说明 |
| --- | --- |
| `Status` | legacy 中为 true 时可能需要展示引导 |
| `Message` | 服务端提示语 |
...
3. 清理 URL 时保留合法 `returnTo`，禁止开放重定向。
4. 只允许站内路径作为 `returnTo`。
...
禁止记录：`Code`、`dingtalkcode`、`Ticket`、`Token`、完整请求 URL 和签名。
