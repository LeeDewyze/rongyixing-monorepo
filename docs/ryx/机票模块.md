...
### 2.1.1 因私 public 机票对照

legacy 因私机票不在 `tmc-flight/` 目录内，而在 `public/public-flight/` 下实现；H5 迁移时可以复用新页面 UI，但业务行为应按 public 链路对齐。

| 路由 / 文件 | 职责 | 关键行为 |
| --- | --- | --- |
| `public-flight-search_ryx` | 因私机票搜索入口 | 选择乘机人、出发到达、日期；机场资源仍复用 `TmcApiHomeUrl-Resource-Airport` |
| `public-flight-list_ryx` | 因私机票列表 | 查询 `TmcTouristFlightUrl-Home-Index`；不调用因公 `Home-Policy` |
| `public-flight-item-cabins_ryx` | 因私舱位选择 | 查询 `TmcTouristFlightUrl-Home-Detail`；选舱后写入 `PublicFlightService.passengerBookInfos` 并跳填单 |
| `public-flight-book_ryx` | 因私填单 / 提交 / 支付前置 | 初始化、联系人、保险、Validate、Book、CheckPay、进入因私支付 |
| `public-flight-passenger-list_ryx` | 因私乘机人选择 | 添加 / 移除 / 修改常旅客 |
| `public-order-list` | 因私订单列表 | legacy 因私机票下单后默认回到这里，`tabId=plane&doRefresh=true` |

因私机票填单的关键差异：

1. 填单初始化走 `TmcTouristBookUrl-Flight-Initialize`，由 `PublicService.addTouristTmcMmsIds()` 注入 tourist `TmcId/MmsId`。
2. Initialize 返回的 `Linkman` 会自动带入订单级联系人卡片；普通预订提交时必须写入 `bookDto.Linkmans = [orderLinkman]`。
3. 普通因私填单页不展示因公的审批、组织、成本中心、超标原因、出差单号，也不走 `TmcApiFlightUrl-Home-Policy`。
4. Initialize 返回 `Insurances` 时展示保险单选；选中项随乘客写到 `Passenger.FlightCabin.InsuranceProducts`。
5. Initialize 返回 `PayTypes` 会保存为支付候选，但普通填单页不展示支付方式单选；支付渠道在 `PublicOrderService.payOrder()` 中通过 `TmcTouristOrderUrl-Order-GetOrderPays` 再取。
6. 提交前校验乘客、联系人姓名、联系人手机号、手机号格式；邮箱格式校验代码存在但被注释，实际不强制。
7. 协议价 / 军警类舱位会先弹 `ValidateComComponent`，调用 `TmcTouristBookUrl-Flight-Validate` 做服务端校验。
8. 往返提交前校验回程起飞时间必须晚于去程到达时间。
9. `Flight-Book` 返回 `IsCheckPay=true` 时显示「正在预订中，请稍候...」，最多 5 次、每 3 秒轮询 `TmcTouristBookUrl-Flight-CheckPay`。
10. `CheckPay` 成功且无审批任务时调用 `PublicOrderService.payOrder({ orderId: TradeNo })`；支付域为 `TmcTouristOrderUrl-*`，最终跳支付域或打开支付页。
11. legacy 因私机票下单后清空已选乘客 / 航班缓存，并回到因私订单列表；H5 如改为订单详情，需要确保状态、支付入口和 channel 都与后续链路一致。

当前 H5 `/flight/book?channel=tourist` 仍需补齐：订单级联系人及提交字段、统一业务弹框、下单后 `CheckPay` loading 与因私支付衔接、旅客添加 / 移除入口、因私保险展示与提交、`Flight-Validate`、往返时间校验。
...
| 政策校验   | `policyFlights`, `FlightPolicy.Rules`                   | 乘客 + 航班/舱位 + TravelFormId           | `Home-Policy`                                   |
...
### 4.3.1 Legacy 差标管理（因公机票）

机票差标规则不在前端维护。legacy 前端只负责把航班、舱位、乘客和出差单上下文提交给 `TmcApiFlightUrl-Home-Policy`，再按后端返回的 `FlightPolicies` 控制舱位展示、预订拦截和下单字段。

**适用范围**

- 因公机票走 `TmcApiFlightUrl-Home-Index` / `Home-Detail` / `Home-Policy`。
- 因私 public 机票走 `TmcTouristFlightUrl-Home-Index` / `Home-Detail`，legacy 未走 `Home-Policy`；H5 不应在因私链路发起因公差标接口。

**Policy 请求**

`getPolicyflightsAsync()` 组装 `TmcApiFlightUrl-Home-Policy`：

| 字段 | 来源与含义 |
| ---- | ---------- |
| `Flights` | `flightResult.Result` 序列化；legacy 会去掉 `totalSegments`、`flightListResult`、`detailResult*`、`transferSegments` 等重字段，并保留 `FlightSegments[].Cabins` |
| `Passengers` | 去重后的乘客 / 当前账号 `AccountId`，逗号拼接 |
| `FlightDetail` | `currentViewtFlightSegment.detailResultForVerify` 序列化，用于精确判断舱位政策 |
| `TravelFromId` | 选中乘客携带的出差单 ID，多个去重后逗号拼接 |

**白名单 / 非白名单**

- 白名单乘客：用乘客自己的 `AccountId` 查差标。
- TMC 配置 `VariablesObj.CheckPolicyUrl` 存在时：把所有乘客都纳入差标检查。
- 非白名单乘客：legacy 当前逻辑不是直接放开，而是先用当前登录人 `Member.Id` 查询差标，再把返回的 `FlightPolicies` 挂到非白名单乘客的 `PassengerKey` 下。
- 若有白名单乘客但最终 `policyFlights` 为空，legacy 视为差标失败，返回空结果。

**舱位匹配与颜色**

`filterPassengerPolicyCabins()` 先把当前航段 `Cabins` 转成默认可订 policy 行，再在选中筛选乘客 `isFilterPolicy=true` 时按 `PassengerKey == passenger.AccountId` 取 `FlightPolicies`，并以 `FlightNo == flightSegment.Number`、`Id == Cabin.Id` 匹配舱位。

| 状态 | legacy 颜色 | 行为 |
| ---- | ----------- | ---- |
| 无 `Rules` 且无 `Descriptions` | `success` | 符合差标 |
| 有 `Rules` 或 `Descriptions` | `warning` | 超标但允许继续 |
| `IsAllowBook=false` | `danger` | 非代理不可预订 |
| 未过滤 / 未命中 | `secondary` | 默认普通可订态 |

默认筛选乘客规则在舱位页 `setDefaultFilteredInfo()`：自订或只有 1 人时过滤第一个；多人代订时通常过滤未选航班的乘客或正在重选的乘客。用户可通过 `FilterPassengersPolicyComponent` 切换过滤乘客，也可取消过滤。

**预订拦截**

点预订时 `onBookTicket()` 会确保 `policyFlights` 已存在；若为空则调用 `initFlightSegmentCabinsPolicy()` 重新计算。仍为空时提示“差标获取失败”，非代理中断，代理可继续。对具体乘客选舱时，`getPolicyCabinBookInfo()` 将 `FlightPolicy` 写入 `bookInfo.flightPolicy`；若 `IsAllowBook=false`，非代理提示“超标不可预订”并清掉该乘客 `bookInfo`，代理可继续但保留超标提示。

**下单提交与审批**

填单提交时每位乘客 DTO 带回：

- `IllegalPolicy = flightPolicy.Rules.join(",")`
- `IllegalReason = otherIllegalReason || illegalReason || ""`
- `FlightCabin = flightPolicy.Cabin`
- `Policy = passenger.Policy`
- `TravelPayType`、`IsSkipApprove`、成本中心、组织、审批人等因公字段

若舱位有 `Rules` 且 `Tmc.IsNeedIllegalReason=true`，legacy 要求填写超标原因。审批展示由 `FlightApprovalType` 控制：`None` 不展示；`Free` / `Approver` 按配置展示；`ExceedPolicyFree` / `ExceedPolicyApprover` 只在超标时触发，且非白名单在部分超标审批模式下不显示审批人。

**H5 迁移对齐结论**

- 因公机票才调用 `TmcApiFlightUrl-Home-Policy`；因私机票应保持 public 链路，只调用 tourist flight 列表 / 详情 / 预订域接口。
- H5 不在前端重建差标规则，只做请求上下文传递、返回结果映射、不可订拦截、超标原因收集和下单字段回传。
- 因私链路没有 legacy 实际生效的 `Home-Policy`，因此因私舱位页不能因为缺少 `policyFlights` 阻断进入预订。
- 对照 legacy 时优先看 `tmc-flight.service.ts` 的 `loadPolicyedFlightsAsync()`、`getPolicyflightsAsync()`、`filterPassengerPolicyCabins()`、`getPolicyCabinBookInfo()`，以及 `public-flight.service.ts` 的 public 链路实现。
...
| `TmcApiFlightUrl-Home-Policy`         | `getPolicyflightsAsync()`             | **差标/违标**     | `Flights`, `Passengers`, `FlightDetail`, `TravelFromId`                                    |
...
支付与公私链路的完整规则见 [Legacy 订单支付流程梳理](../api/domains/order-payment-legacy-flow.md)：因私订单使用 tourist 支付域，因公订单使用 TMC 支付域；因公公付款不进入个人支付页。
...
| `tmc-flight-item-cabins_ryx`        | `/flight/:flightId/cabins` | 部分，Policy/多人未完整；预订为占位 |
| `tmc-flight-selected-bookinfos_ryx` | （不迁）                   | Phase C **单程直达填单**，跳过汇总页；往返不在范围 |
| `tmc-flight-book_ryx`               | `/flight/book`（规划中）   | 未迁 — **Phase C**，见下方策略      |
...
**填单页 Phase C** 迁移策略：[flight-book-migration-strategy.md](../api/domains/flight-book-migration-strategy.md)
