...
### 2.4.1 因私 public 火车对照

legacy 因私火车不在 `tmc-train/` 目录内，而在 `public/public-train/` 下实现；H5 迁移时复用新页面 UI，但业务行为应按 public 链路对齐。

| legacy 文件 / 路由 | 职责 | H5 迁移关注点 |
| ------------------ | ---- | -------------- |
| `public-train-search_ryx` | 因私火车搜索入口 | 搜索走 `TmcTouristTrainUrl-Home-Search` |
| `public-train-list_ryx` | 因私车次列表、席别选择、时刻表 | 时刻表走 `TmcTouristTrainUrl-Home-Schedule`；不走因公 Policy 阻断 |
| `public-train-book_ryx` | 因私填单、12306、选座、联系人、提交 | 初始化 / 下单走 `TmcTouristBookUrl-Train-Initialize/Book` |
| `public-train-freq-passenger-list` | 因私常旅客选择 | 旅客上限为 5 人，超过提示“人数不能超过5人” |
| `public-train-bookinfos` | 金额 / 支付确认 | 普通填单页不展示支付方式选择，确认页再使用初始化 `PayTypes[0]` |

public 火车填单关键差异：

1. 旅客来源是 public frequent passenger，非因公员工 / 出差单乘客体系。
2. 12306 绑定、解绑、账号校验、验证码校验、联系人均使用 `TmcTouristBookUrl-Train-*`。
3. 填单页不展示因公支付方式单选，不展示审批、组织、成本中心、超标原因、TravelNumber。
4. 选座是页面级整体偏好池，不是每位旅客独立选座；legacy 渲染两排座位，提交时按旅客顺序写入 `Train.BookSeatLocation`。
5. 选座仅适用于二等座、一等座、商务座等支持选座席别；H5 同时兼容特等座布局，硬座 / 卧铺不展示。
6. 联系人默认值来自 Initialize 返回的 `Linkman`，H5 personal 火车提交为订单级 `Linkmans`。
...
### 4.4.1 Legacy 差标管理（因公火车）

火车票差标规则不在前端维护。legacy 前端负责用车次、席别、乘客和出差单上下文调用 `TmcApiTrainUrl-Home-Policy`，再按返回的 `TrainPolicies` 标记席别、拦截不可订席别，并在下单时带回违规信息。

**适用范围**

- 因公火车走 `TmcApiTrainUrl-Home-Search` / `Home-Policy`。
- 因私 public 火车走 `TmcTouristTrainUrl-Home-Search` / `Home-Schedule` / tourist book 域；legacy 中 `TmcTouristTrainUrl-Home-Policy` 代码处于注释状态，实际 public 火车不走因公差标阻断。

**Policy 请求**

`policyAsync()` 组装 `TmcApiTrainUrl-Home-Policy`，`Version=2.0`，`Timeout=60`：

| 字段 | 来源与含义 |
| ---- | ---------- |
| `Passengers` | 去重后的乘客 / 当前账号 `AccountId`，逗号拼接 |
| `Trains` | `Home-Search` 返回的 `TrainEntity[]` 序列化，包含每个车次的 `Seats` 供后端按席别判断 |
| `TravelFromId` | 选中乘客携带的出差单 ID，多个去重后逗号拼接 |

**白名单 / 非白名单**

- 白名单乘客：用乘客自己的 `AccountId` 查差标。
- 非白名单乘客：legacy 当前逻辑用当前登录人 `Member.Id` 查询差标，再把返回的 `TrainPolicies` 挂到非白名单乘客的 `PassengerKey` 下。
- TMC 配置 `VariablesObj.CheckPolicyUrl` 存在时：会把所有乘客都纳入差标检查。
- 如果有乘客但最终 `totalPolicies` 为空，legacy 将车次列表置空并中断展示，避免在无法确认差标时继续预订。

**席别匹配与颜色**

`filterPassengerPolicyTrains()` 先把所有席别颜色重置为 `secondary`，再在当前筛选乘客 `isFilterPolicy=true` 时按 `PassengerKey == passenger.AccountId` 取 `TrainPolicies`，并以 `TrainNo == train.TrainNo && SeatType == seat.SeatType` 匹配席别。

| 状态 | legacy 颜色 | 行为 |
| ---- | ----------- | ---- |
| 无 `Rules` | `success` | 符合差标 |
| 有 `Rules` | `warning` | 超标但允许继续 |
| `IsAllowBook=false` | `danger` | 非代理不可预订 |
| 未过滤 / 未命中 | `secondary` | 默认普通可订态 |

卧铺上/中/下等细分席别在 H5 需要归一到 legacy 策略席别组后再匹配，避免同一类卧铺因为 `SeatType` 细分导致差标丢失。

**预订拦截**

列表页选席别时，legacy 会用 `checkIfSeatIsAllowBook()` / `getPolicyTrainBookInfo()` 判断当前乘客和席别是否可订。非代理遇到 `IsAllowBook=false` 时提示“超标不可预订”并阻断；代理可继续，但需要保留超标状态用于下单展示和审批。

**下单提交与审批**

填单提交时每位乘客 DTO 带回：

- `IllegalPolicy = trainPolicy.Rules.join(",")`
- `IllegalReason = otherIllegalReason || illegalReason || ""`
- `Train` / `Train.BookSeatType` / `Train.BookSeatLocation`
- `Policy = passenger.Policy`
- `TravelPayType`、`IsSkipApprove`、组织、成本中心、审批人等因公字段

若 `trainPolicy.Rules.length > 0` 且 `Tmc.IsNeedIllegalReason=true`，legacy 要求填写超标原因；改签场景下此必填逻辑会被放宽。审批展示由 `TrainApprovalType` / `tmcService.isShowApprove("Train", hasRules)` 决定，超标审批模式只在有 `Rules` 时触发。

**H5 迁移对齐结论**

- 因公火车才调用 `TmcApiTrainUrl-Home-Policy`；因私 public 火车保持 tourist train / tourist book 域，不走因公差标阻断。
- H5 不在前端维护差标规则，只负责把车次、席别、乘客、出差单上下文传给后端，并按返回的 `TrainPolicies` 做展示、拦截、超标原因和下单字段回传。
- 火车席别匹配要特别注意卧铺等细分席别归一，否则会出现后端有差标结果但 H5 匹配不到的情况。
- 对照 legacy 时优先看 `tmc-train.service.ts` 的 `loadPolicyedTrainsAsync()`、`policyAsync()`、`filterPassengerPolicyTrains()`，以及 `public-train.service.ts` 的 public 链路实现。
...
支付与公私链路的完整规则见 [Legacy 订单支付流程梳理](../api/domains/order-payment-legacy-flow.md)：因私走 `TmcTouristOrderUrl-*`，因公走 `TmcApiOrderUrl-*`；因公 `Company` / `Balance` 不进入个人支付页，`Person` / `Credit` 才进入支付链路。

填单页个付时限文案迁移规则：legacy 模板固定展示“20分钟”，H5 为与服务端配置保持一致，优先使用初始化返回的 `Tmc.TrainHoldMinute`，无值时兜底 20 分钟。

### 4.7.1 下单失败提示与弹框承接

火车填单页提交 `Train-Book` 失败时，H5 不应将服务端业务错误改写成本地校验文案，而是通过 `formatApiError(error, "train")` 保留后端 `Message` 并展示。因公提交接口为 `TmcApiBookUrl-Train-Book`，因私提交接口为 `TmcTouristBookUrl-Train-Book`。

示例：`乘客存在重复行程，请重新预订` 属于服务端 `Train-Book` 返回的业务失败提示，不是 H5 本地硬编码校验。当前 H5 的承接弹框为 `PassengerSelectAlertDialog`，标题固定为“温馨提示”，普通业务错误原文展示并由用户点击“确定”关闭。

本地提交前校验与服务端业务失败需要分层处理：手机号重复、联系人手机号重复、必填字段等属于 H5 提交前校验；重复行程、价格/库存/签名校验等以后端返回为准，前端只负责透传提示和中断后续跳转。
...
| ---------------------------- | ------------------- | --------------------------------------------- |
| `tmc-train-search_ryx`       | `/train`            | 部分（缺往返/12306）                          |
| `tmc-train-list_ryx`         | `/train/list`       | 部分（Policy/筛选/经停站/改签模式已接）       |
| `tmc-train-book_ryx`         | `/train/book`       | 已迁（含 ExchangeBook、checkPay→支付）        |
| 选站浮层 / 独立选站页        | （待建）            | 未迁                                          |
| 下单后                       | 订单详情 / 支付     | 已迁（`/orders/train/:id`、`/train/pay/:id`） |
| `tmc-order-train-detail_ryx` | `/orders/train/:id` | 已迁（退票/改签/出票/经停站/联系人）          |

新 monorepo 当前 API 封装见 `packages/api/src/apis/train.ts`、`packages/api/src/apis/order.ts`。完整页面对照见 [PAGE-API-MATRIX.md](../api/PAGE-API-MATRIX.md)。
