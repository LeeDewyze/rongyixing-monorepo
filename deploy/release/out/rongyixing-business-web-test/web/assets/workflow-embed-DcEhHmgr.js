import{r as i,i as s,a as l}from"./workflow-site-CoHDEENr.js";function E(e){try{const t=i(e);return s(t)&&new URL(t).hostname===l()}catch{return!1}}function a(e){if(e===!0||e===1)return!0;if(typeof e=="string"){const t=e.trim().toLowerCase();return t==="true"||t==="1"||t==="yes"}return!!e}function c(e){if(!e)return null;if(typeof e=="string")try{const t=JSON.parse(e);return typeof t=="object"&&t!==null?t:null}catch{return null}return typeof e=="object"?e:null}function C(e){const t=c(e);if(!t||typeof t.type!="string")return!1;const o=t.type;if(o==="windowclose")return!0;const n=a(t.isBack)||a(t.payload)||a(t.data);return o==="back"||o==="appCheckGoBack"||o==="appCheckCanBack"?n:!1}function S(e){const t=c(e);return!t||t.type!=="ryxEmbedOverlay"?null:{open:a(t.open)}}function g(e,t){const o=t.startsWith("?")?t.slice(1):t,n=`<script type="text/javascript">
(function () {
...
<\/script>`;return e.includes("<head>")?e.replace("<head>",`<head>${n}`):`${n}${e}`}function x(e){const t=`<script type="text/javascript">
(function () {
...
<\/script>`;return e.includes("<head>")?e.replace("<head>",`<head>${t}`):`${t}${e}`}function b(e,t){if(!t||/window\.ticket\s*=/.test(e))return e;const o=`window.ticket = ${JSON.stringify(t)};`,n=`<script type="text/javascript">${o}<\/script>`;if(e.includes("<head>"))return e.replace("<head>",`<head>${n}`);const r=e.match(/window\.FormNumber\s*=\s*"[^"]*";/);return r?e.replace(r[0],`${r[0]}
    ...
    <script type="text/javascript" src="/js/detail.js">`)}function h(e){const t=`<style data-ryx-embed-scroll="true">
html, body {
...
<\/script>`;return e.includes("<head>")?e.replace("<head>",`<head>${t}`):`${t}${e}`}function v(e,t){const o=`<base href="${t.replace(/\/$/,"")}/">`;return e.includes("<head>")?e.replace("<head>",`<head>${o}`):`${o}${e}`}async function _(e){const t=i(e);let o;try{o=new URL(t)}catch{return}if(!s(t)||o.hostname!==l())return;const n=o.searchParams.get("ticket")??"";if(!n)return;const r=await fetch(t);if(!r.ok)throw new Error(`workflow embed fetch failed: HTTP ${r.status}`);const d=await r.text(),p=`${o.protocol}//${o.host}`,f=o.search.startsWith("?")?o.search.slice(1):o.search,u=g(d,f),y=x(u),m=b(y,n),w=h(m);return v(w,p)}export{C as a,S as b,_ as f,E as i};
