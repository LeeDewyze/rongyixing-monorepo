...
<\/script>`;return e.includes("<head>")?e.replace("<head>",`<head>${t}`):`${t}${e}`}function b(e,t){if(!t||/window\.ticket\s*=/.test(e))return e;const r=`window.ticket = ${JSON.stringify(t)};`,o=`<script type="text/javascript">${r}<\/script>`;if(e.includes("<head>"))return e.replace("<head>",`<head>${o}`);const n=e.match(/window\.FormNumber\s*=\s*"[^"]*";/);return n?e.replace(n[0],`${n[0]}
        ${r}`):e.replace('<script type="text/javascript" src="/js/detail.js">',`<script type="text/javascript">${r}<\/script>
    <script type="text/javascript" src="/js/detail.js">`)}function h(e,t){const r=`<base href="${t.replace(/\/$/,"")}/">`;return e.includes("<head>")?e.replace("<head>",`<head>${r}`):`${r}${e}`}async function _(e){const t=i(e);let r;try{r=new URL(t)}catch{return}if(!s(t)||r.hostname!==c())return;const o=r.searchParams.get("ticket")??"";if(!o)return;const n=await fetch(t);if(!n.ok)throw new Error(`workflow embed fetch failed: HTTP ${n.status}`);const l=await n.text(),d=`${r.protocol}//${r.host}`,f=r.search.startsWith("?")?r.search.slice(1):r.search,u=g(l,f),p=m(u),y=b(p,o);return h(y,d)}export{v as a,_ as f,x as i};
