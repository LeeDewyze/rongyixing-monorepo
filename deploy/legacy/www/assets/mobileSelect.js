...
(function () {
	...
	MobileSelect.prototype = {
		...
		addListenerWheel: function (theWheel, index) {
			//会告诉浏览器你不想阻止事件的默认行为，而prevent会告诉浏览器阻止事件的默认行为。因此passive和prevent冲突，不能同时绑定在一个监听器上
			var _this = this;
		...
		getCurValue: function () {
			...
			if (_this.cascade) {
				...
				for (var i = 0; i < _this.curDistance.length; i++) {
					const html=_this.getInnerHtml(i);
					temp.push(html);
				}
			}
			return temp;
		...
		getInnerHtml: function (sliderIndex) {
			...
			const el=_this.slider[sliderIndex].getElementsByTagName('li')[index];
		...
		touch: function (event, theSlider, index) {
			...
			switch (event.type) {
				...
				case "touchend":
					...
					if(_this.config.onTouchEnd){
						_this.config.onTouchEnd(_this.getCurValue(), index)
					}
					break;
				...
				case "touchmove":
					...
					if(_this.config.onTouchMove){
						_this.config.onTouchMove(_this.getCurValue(), index)
					}
					break;
			}
		},
