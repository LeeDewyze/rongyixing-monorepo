...
describe("order-routes", () => {
  ...
  it("routes flight detail and pay paths", () => {
    ...
    expect(getOrderResultPath("Flight", "ORD-FLT-001")).toBe("/orders/flight/ORD-FLT-001");
  });
  ...
  it("routes train detail and pay paths", () => {
    ...
    expect(getOrderResultPath("Train", "ORD-TRN-001")).toBe("/orders/train/ORD-TRN-001");
  });

  it("builds order list path with channel and scope", () => {
    expect(getOrderListPath("train", { channel: "tourist", scope: "pendingTravel" })).toBe(
      "/orders?channel=tourist&tab=train&scope=pendingTravel",
    );
  });
});
