...
async function handleHttpUrl(
  ...
  if (props.isOpenInAppBrowser || props.isBlank) {
    window.open(openUrl, props.isBlank ? "_blank" : "_self");
    return true;
  }
...
/** Port of legacy `CoreHelper.jump` for web react-router navigation. */
    ...
    try {
      ...
      if (jumpInfo.wechatMiniAppId?.trim() && jumpInfo.wechatMiniPath?.trim()) {
        if (import.meta.env.DEV) {
          console.warn("[coreJump] WeChat mini jump skipped on web");
        }
        return false;
      }
