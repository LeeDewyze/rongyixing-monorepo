...
export function TrainBookPage() {
  ...
  async function executeSubmit() {
    ...
    try {
          ...
          navigate(payPath, {
            ...
            state: {
              bookingHandoff: {
                product: "train",
                target: payPath,
                clearExchangeSession: isExchange,
              },
            },
          });
          return;
      ...
      const target = `/home/orders?tab=${TAB_ID_TO_PARAM.train}`;
      navigate(target, {
        replace: true,
        flushSync: true,
        state: {
  ...
  if (initBook.isLoading) {
    return (
      <div className={WEB_PAGE_ROOT} style={TRAIN_BOOK_PAGE_BACKGROUND}>
        <TrainBookHeader title={isExchangeBook ? "确认信息" : undefined} />
        <div
          ref={contentRef}
