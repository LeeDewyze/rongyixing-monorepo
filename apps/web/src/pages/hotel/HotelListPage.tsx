...
export function HotelListPage() {
  ...
  useEffect(() => {
    if (!hasParams) navigate("/?product=hotel", { replace: true });
  }, [hasParams, navigate]);
  ...
  function openDetail(hotel: (typeof hotels)[number]) {
    ...
    if (hotel.Star) params.set("star", String(hotel.Star));
  ...
  return (
    <div className={WEB_PAGE_ROOT} style={{ background: "var(--brand-form-header-gradient)" }}>
      <div
        ref={headerRef}
        className={WEB_PAGE_STICKY_HEADER}
      ...
      >
        <HotelListHeader
          ...
          checkOut={checkOut}
          keyword={keyword}
          onBack={() => navigateBack(navigate, "/?product=hotel")}
      ...
      <div ref={handleScrollRoot} className={WEB_PAGE_BODY}>
        <div
        ...
        <div className="relative z-0 space-y-2 px-3 pb-3 pt-1">
