...
interface CityPickerDialogProps<T> extends CityPickerAdapter<T> {
  open: boolean;
  items: T[];
  historyKey: string;
  onClose: () => void;
...
function CityChip({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      className="flex min-h-11 items-center justify-center rounded-lg border border-black/[0.06] bg-white px-2 py-2.5 text-sm text-[#333333] shadow-sm hover:bg-[#f5f5f5] pointer-coarse:min-h-12"
      onClick={onClick}
    >
      {label}
    </button>
...
function PickerListRow<T>({
  ...
  return (
    <button
      type="button"
      className="flex min-h-11 w-full items-center border-b border-[#eeeeee] bg-white px-4 py-3 text-left text-sm text-[#333333] hover:bg-[#f5f5f5] pointer-coarse:py-4"
      onClick={() => onSelect(row.item)}
    >
      <span className="flex-1 truncate">
        <span>{row.name}</span>
        {showCode && row.code ? <span className="text-[#999999]"> ({row.code})</span> : null}
      </span>
    </button>
  );
}
...
function SearchResultRow<T>({
  ...
  return (
    <button
      type="button"
      className="flex min-h-11 w-full items-start gap-2 border-b border-[#eeeeee] bg-white px-4 py-3 text-left hover:bg-[#f5f5f5] pointer-coarse:py-4"
      onClick={() => onSelect(row.item)}
    >
...
/** Pad/PC centered city/station picker dialog. */
export function CityPickerDialog<T>({
  open,
  items,
  historyKey,
  onClose,
  ...
  const normalizedAll = useMemo(() => normalizePickerItems(items, adapter), [items, adapter]);
  ...
  const hotItems = useMemo(() => browseItems.filter((row) => row.isHot), [browseItems]);

  const { letters, groups } = useMemo(() => groupByFirstLetter(browseItems), [browseItems]);
  ...
  const hotGridClass =
    hotGridColumns === 2 ? "grid-cols-2" : hotGridColumns === 4 ? "grid-cols-4" : "grid-cols-3";
        ...
        <DialogHeader className="shrink-0 border-b px-4 py-3">
          ...
          <div className="mt-2">
            <input
              ...
              className="min-h-11 w-full rounded-lg border border-input bg-[#F5F6F9] px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50 pointer-coarse:min-h-12"
            />
          </div>
        </DialogHeader>
            ...
            <div className="pointer-events-none absolute inset-y-0 right-1 z-10 flex w-5 flex-col items-center justify-center gap-0.5">
              {letters.map((letter) => (
                <button
                  key={letter}
                  type="button"
                  className="pointer-events-auto min-h-4 text-[11px] leading-3 text-[#999999] hover:text-brand-accent"
