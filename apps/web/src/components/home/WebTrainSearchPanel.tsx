...
interface WebTrainSearchPanelProps {
...
export function WebTrainSearchPanel({
  fromStation,
  toStation,
  date,
  validationError,
      ...
      <div className="flex items-stretch gap-3">
        <div className="flex h-16 min-w-0 flex-1 items-center gap-3 rounded-xl bg-[#F5F6F9] px-4">
          <button type="button" className="min-w-0 flex-1 text-left" onClick={onSelectFrom}>
            <span className={FIELD_LABEL_CLASS}>出发站</span>
            <span className={FIELD_VALUE_CLASS}>{displayStationName(fromStation)}</span>
          </button>
          ...
          <button type="button" className="min-w-0 flex-1 text-right" onClick={onSelectTo}>
            <span className={FIELD_LABEL_CLASS}>到达站</span>
            <span className={FIELD_VALUE_CLASS}>{displayStationName(toStation)}</span>
          </button>
        </div>
