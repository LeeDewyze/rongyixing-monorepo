...
interface FlightOrderPassengerTabsProps {
  tickets: FlightOrderTicket[];
  selectedIndex: number;
  onSelect: (index: number) => void;
}

export function FlightOrderPassengerTabs({
  tickets,
  selectedIndex,
  onSelect,
}: FlightOrderPassengerTabsProps) {
  if (tickets.length <= 1) {
    return null;
  }
  ...
  return (
    <div className={`flex gap-2 overflow-x-auto px-0 py-1 ${HOTEL_DETAIL_FONT}`}>
      {tickets.map((ticket, index) => {
        ...
        const label = ticket.Traveler?.Name ?? `乘机人${index + 1}`;
        return (
          <button
            key={ticket.Id}
            type="button"
