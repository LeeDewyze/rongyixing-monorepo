import type { HotelOrderBillLine, TrainOrderTicket } from "@ryx/shared-types";

import { FareRulesBottomSheet } from "@/components/flight/flight-fare-rule-presentation";
import { sumBillLines } from "@/lib/train-order-detail";

interface TrainOrderBillSheetProps {
  open: boolean;
...
export function TrainOrderBillSheet({ open, ticket, lines, onClose }: TrainOrderBillSheetProps) {
  ...
  return (
    <FareRulesBottomSheet
      open={open}
      title="账单明细"
      titleId="train-order-bill-title"
      onClose={onClose}
    >
      {!ticket ? (
          ...
          <div className="border-b border-[#EEF1F6] bg-white/70 px-3.5 py-2.5">
            ...
            {transactionId ? (
              <p className="mt-1 text-[12px] leading-snug text-[#999999]">
                ...
                {statusName ? <span className="text-[#999999]"> ({statusName})</span> : null}
              </p>
            ) : null}
          </div>
