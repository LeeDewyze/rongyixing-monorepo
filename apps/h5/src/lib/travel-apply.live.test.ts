...
describe("travel apply live (staging)", () => {
  it("submits multi-traveler multi-segment form for T18610773065", async () => {
    ...
    const segmentA = {
      ...
      fromCity: meta.cities.find((city) => city.label === "北京") ?? meta.cities[0]!,
      toCity:
        meta.cities.find((city) => city.label === "上海") ?? meta.cities[1] ?? meta.cities[0]!,
    };
    const segmentB = {
      ...defaultTravelApplySegment(meta.cities),
    ...
    const values: TravelApplyFormValues = {
      ...
      travelers: [defaultTravelApplyTraveler(meta.defaultAccount), { account: secondStaff }],
